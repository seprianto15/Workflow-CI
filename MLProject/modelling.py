...
def main():
    ...
    # 4. Manajemen Sesi Run MLflow
    active_run = mlflow.active_run()

    if active_run:
        current_run_id = active_run.info.run_id
        print(f"Memulai MLflow run dengan ID aktif: {current_run_id}")
        ...
        # 5. Ekspor Run ID ke base_dir (direktori MLProject) agar dapat ditarik oleh CI/CD GitHub Actions
        txt_path = os.path.join(base_dir, 'run_id.txt')
        with open(txt_path, 'w') as f:
            f.write(current_run_id)
        print(f"Run ID successfully written to {txt_path}")
